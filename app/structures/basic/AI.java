...
/*The AI strategy uses effect cards first and directly reduces the number of players first. 
 * The stun card acts on the unit with the highest attack power. 
 * Healing cards are used for the unit that has lost the most health, 
 * or the unit with the highest attack power, or the unit that is most vulnerable to attack. 
 * Attack cards are used when the current health value is less than
 */
...
{for(Card cur:spellCard)
	...
	{switch(curname)
	{
	case "Sundrop Elixir":useSundrop(cur,gameState, out, message);//Optimize the timing of use later
		break;
	case "True Strike":useTrueStrike(cur,gameState, out, message);
		break;
	case "Beam Shock":useStun(cur,gameState, out, message);//Later upgrade to use bean shock after attacking once
		break;
	}
	}
	}
...
{if(!gameState.getUserPlayer().getAllUnitsAndTile().get(cur).getStunned()&&gameState.getUserPlayer().getAllUnitsAndTile().get(cur).getAttack()>gameState.getUserPlayer().getAllUnitsAndTile().get(attackMax).getAttack())
{attackMax=cur;
	}
	}
...
int maxLostHp=0;
for(Tile cur:gameState.getUserPlayer().getAllUnitsAndTile().keySet())
{int nowLostHp=gameState.getUserPlayer().getAllUnitsAndTile().get(cur).getMaxHealth()-gameState.getUserPlayer().getAllUnits().get(cur).getCurHealth();
...
{Tile healthMin=null;
for(Tile cur:gameState.getUserPlayer().getAllUnitsAndTile().keySet())
{if(gameState.getUserPlayer().getAllUnitsAndTile().get(cur).getCurHealth()<gameState.getUserPlayer().getAllUnitsAndTile().get(healthMin).getCurHealth())
{healthMin=cur;
	}
	}
...
{
	Collections.sort(this.getHandCards(), new PersonComparator());
	for(Card cur:this.getHandCards())
	{if(this.getMana()<cur.getManacost())
	{
		return;
...
public Tile getClosestTile(Tile aitile,GameState gameState)//Ensure that the first round has been completed for a week to determine whether there is a provoke card.
{  int rows = 9;//Rows
int cols = 5;//columns
...
while (!queue.isEmpty()) {
    ...
    if (k>8&&gameState.getUserPlayer().getAllUnitsAndTile().containsKey(gameState.getTileByPos(currentRow,currentCol))) {
    ...
    if(gameState.getUserPlayer().getAllUnitsAndTile().containsKey(gameState.getTileByPos(currentRow,currentCol))&&gameState.getUserPlayer().getAllUnitsAndTile().get(gameState.getTileByPos(currentRow,currentCol)).getId()==1)
    {
    	return gameState.getTileByPos(currentRow,currentCol);
    }
    if(gameState.getUserPlayer().getAllUnitsAndTile().containsKey(gameState.getTileByPos(currentRow,currentCol))) 
    {
